 ...
 * teamIndexToConference (optional - from parseConferences) enables tagging
 * each game as a conference game (both teams in the same conference that
 * season); omitted, every game is tagged non-conference rather than failing.
 */
async function parseMyTeamGames(franchise, myTeamIndex, teamIndexToConference) {
    ...
    for (const r of archiveTable.records) {
        ...
        const isNationalChampionship = r.SeasonWeekType === 'NationalChampionship';
        const isPlayoff = isPlayoffBowl || isNationalChampionship;

        const isConference = !!(teamIndexToConference &&
            teamIndexToConference.get(myTeamIndex) != null &&
            teamIndexToConference.get(myTeamIndex) === teamIndexToConference.get(opponentTeamIndex));
