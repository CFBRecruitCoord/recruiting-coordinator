...
// The Recruit table's own Class field covers three populations sharing one
// table/schema: true high school recruits ("HighSchool"), junior college
// recruits ("JuniorCollege_*"), and transfer portal players
// ("Transfer_Freshman/Sophomore/Junior" - named for the class they're
// transferring in as). Verified live: a save before the transfer portal
// opens has zero Transfer_* records at all (not an empty/broken feature -
// the portal just hasn't opened yet), so an empty transfer pool is a real,
// expected state to handle, not a bug.
function isTransferClass(classValue) {
    return typeof classValue === 'string' && classValue.startsWith('Transfer');
}
...
async function parseRecruits(franchiseOrPath) {
    return parseRecruitPool(franchiseOrPath, r => !isTransferClass(r.Class));
}

/**
 * Same rating math as parseRecruits, scoped to transfer portal players only
 * (used by the Transfer Explorer tab) - their Raw Rating/NIL Adjusted
 * Rating baselines are computed from the transfer pool alone, not mixed
 * with high school recruits, the same "compare like to like" approach used
 * elsewhere in the app (e.g. Class Landscape's national averages).
 * @param {string|object} franchiseOrPath
 * @returns {Promise<Array<Object>>}
 */
async function parseTransfers(franchiseOrPath) {
    return parseRecruitPool(franchiseOrPath, r => isTransferClass(r.Class));
}

/**
 * Shared engine behind parseRecruits/parseTransfers - identical field
 * extraction and rating math, differing only in which Recruit records
 * (by Class) are included, so the two populations never silently mix and
 * a future formula change can't drift between them.
 */
async function parseRecruitPool(franchiseOrPath, includeRecord) {
    ...
    const nonEmpty = recruitTable.records.filter(
        r => !r.isEmpty && r.RecruitStage !== 'Invalid' && r.NationalRank > 0 && includeRecord(r)
    );
    ...
    for (const rec of nonEmpty) {
        ...
        all.push({
            // The Recruit table's own row position - a stable join key back
            // to this exact recruit from lib/parseMyRecruitingBoard.js
            // (which only knows a table/row reference, not any of the
            // identity fields below) without needing a second, separate
            // per-recruit read of the save.
            recruitIndex: rec.index,
            rank: rec.NationalRank,
            posRank: rec.PositionRank,
            stateRank: rec.StateRank,
            stage: rec.RecruitStage,
            totalScholarshipOffers: rec.TotalScholarshipOffers || 0,
            ...
            nil: p.BaseNILValue,
            // Raw archetype enum (e.g. "QB_FieldGeneral") - verified to fully
            // match the Coordinator Settings archetype dropdown options for
            // every position. Sent as the raw value rather than a display
            // label so there's one source of truth for the label mapping
            // (ARCHETYPES_BY_POSITION on the frontend), not two that could drift.
            archetype: p.PlayerType || null
        });
    }
...
module.exports = { parseRecruits, parseTransfers };
