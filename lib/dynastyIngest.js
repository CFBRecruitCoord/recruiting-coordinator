    ...
    try {
        ...
        const upsertGame = db.prepare(`
            INSERT INTO dynasty_games (
                ...
                my_score, opp_score, result, is_bowl, is_playoff, bowl_index,
                is_conference, is_national_championship, created_at
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(user_id, season_year, season_week, opponent_team_index) DO UPDATE SET
                ...
                bowl_index = excluded.bowl_index, is_conference = excluded.is_conference,
                is_national_championship = excluded.is_national_championship
        `);
        let gamesUpserted = 0;
        (games || []).forEach(g => {
            upsertGame.run(
                ...
                g.bowlIndex, g.isConference ? 1 : 0, g.isNationalChampionship ? 1 : 0, new Date().toISOString()
            );
            gamesUpserted++;
        });
