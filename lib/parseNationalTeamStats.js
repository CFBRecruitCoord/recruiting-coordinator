...
const { parseConferences, INDEPENDENTS_LABEL } = require('./parseConferences');
...
async function parseNationalTeamStats(franchise) {
    ...
    const { teamIndexToConference, conferenceNames } = await parseConferences(franchise);
    ...
    for (const r of teamTable.records) {
        ...
        const identity = {
            ...
            conference: teamIndexToConference.get(r.TeamIndex) || INDEPENDENTS_LABEL
        };
    ...
    return { seasonYear: currentYear, seasonWeek: currentWeek, seasonStage: currentStage, currentTeams, historicalSeasons, conferenceNames };
}
