...
function getAuthDb() {
    ...
    db.exec(`
        ...
        -- is_conference/is_national_championship added for the Coaching
        -- Career summary banner: is_conference is computed at ingest time
        -- (not derivable later - needs a live conference-membership lookup,
        -- see parseSchoolRecords.js) by checking whether the opponent shared
        -- my team's conference that season; is_national_championship flags
        -- specifically the title game (SeasonWeekType = 'NationalChampionship'
        -- in the save), a level more specific than is_playoff which already
        -- covers every playoff round.
        CREATE TABLE IF NOT EXISTS dynasty_games (
            ...
            is_national_championship INTEGER DEFAULT 0,
            created_at TEXT NOT NULL,
        ...
        -- Coach tenure: which school the user coached in each dynasty year,
        -- captured every upload from the already-known userTeam/SeasonInfo
        -- (no new parsing needed). This is the source of truth for the
        -- Coaching Career summary banner's "schools coached" timeline, and
        -- for scoping national data (All-Americans, awards, conference
        -- championships) down to "mine, during years I was actually there" -
        -- a coach who changes schools mid-dynasty shouldn't have another
        -- school's players' honors counted as their own.
        CREATE TABLE IF NOT EXISTS dynasty_coach_tenure (
            user_id INTEGER NOT NULL,
            year INTEGER NOT NULL,
            team_index INTEGER NOT NULL,
            computed_at TEXT NOT NULL,
        ...
        -- Conference championship game results, national in scope (every
        -- conference's result, not just the user's) - same reasoning as
        -- dynasty_awards_history: parsed from LeagueHistoryConferenceChampion,
        -- a save table with the same "fixed block of entries per completed
        -- season, no explicit year field" layout as LeagueHistoryAward, so it
        -- needs the same re-ingest-every-upload treatment to keep building
        -- real history rather than being capped by whatever the save itself
        -- currently retains. One row per (year, conference) since multiple
        -- conferences each crown a champion every year.
        CREATE TABLE IF NOT EXISTS dynasty_conference_championships (
            user_id INTEGER NOT NULL,
            year INTEGER NOT NULL,
            conference TEXT NOT NULL,
            winner_team_index INTEGER,
            winner_score INTEGER,
            loser_team_index INTEGER,
            loser_score INTEGER,
            computed_at TEXT NOT NULL,
            PRIMARY KEY (user_id, year, conference)
        );
        ...
        CREATE INDEX IF NOT EXISTS idx_conf_champs_user_year ON dynasty_conference_championships(user_id, year);
    `);
    ...
    // Same reasoning - dynasty_games shipped before is_conference/
    // is_national_championship existed (before the Coaching Career summary
    // banner needed them).
    const dynastyGamesColumns = db.prepare('PRAGMA table_info(dynasty_games)').all().map(c => c.name);
    if (!dynastyGamesColumns.includes('is_conference')) {
        db.exec('ALTER TABLE dynasty_games ADD COLUMN is_conference INTEGER DEFAULT 0');
    }
    if (!dynastyGamesColumns.includes('is_national_championship')) {
        db.exec('ALTER TABLE dynasty_games ADD COLUMN is_national_championship INTEGER DEFAULT 0');
    }
