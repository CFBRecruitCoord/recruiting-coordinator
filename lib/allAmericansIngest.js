 ...
 * lib/awardsIngest.js. Explicitly scoped to team IN ('1st','2nd') - this
 * view predates Freshman All-American capture (added later for the Coaching
 * Career summary) and should stay exactly what it was, not silently start
 * folding freshman selections into "Total" without a matching column.
 */
function getAllAmericanSchoolTotals(userId, scope) {
    const db = getAuthDb();
    const rows = db.prepare("SELECT team_index AS teamIndex, team, year, player_name AS name, position FROM dynasty_all_american_selections WHERE user_id = ? AND scope = ? AND team_index IS NOT NULL AND team IN ('1st', '2nd')").all(userId, scope);
    const teamMeta = teamMetaByIndex(userId);
