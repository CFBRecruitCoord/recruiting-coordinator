 ...
 * 25: Offense/Defense are the only genuinely CALCULATED rankings here (per
 * the user's exact requested inputs - see OFFENSE_METRICS_PLAYED /
 * DEFENSE_METRICS_PLAYED above), scored and ranked independently. The
 * poll-order fields (mediaRank/coachesRank/cfpRank) are passed straight
 * through from the save's own in-game polls, not derived from anything here.
 * compositeRank ("The Coordinator 25") is the one blended field this module
 * produces: the straight average of the save's Media and Coaches poll ranks
 * (both populated from preseason on), folding in the CFP poll too once it's
 * actually active (nonzero for at least one team - it stays unranked
 * league-wide until partway through a real season). Teams with no poll data
 * at all (a completed season predating this feature - see
...
function computeTop25(teams) {
    ...
    const scored = teams.map(t => {
        ...
        return { ...t, isProjected, offenseScore, defenseScore, compositeRank: null };
    });
